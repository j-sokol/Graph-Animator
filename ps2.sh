        ...
        do
            ...
            case $lhs in
                ...
                EFFECTPARAMS ) 
                    ...
                    for i in $rhs; do 
                        if [[ $i =~ ^type=.* ]]; then
                            value=$(echo $i| sed 's;type=;;')
                            if [[ $value != circles ]] && [[ $value != lines ]]; then
                                err "EffectParams value 'type' ($value) has to be set to 'circles' or 'lines'."
                            fi
                            GRAPH_TYPE=$value
                            verbose "GraphType set to $value."
                        fi
                        if [[ $i =~ ^xformat=.* ]]; then
                            value=$(echo $i| sed 's;xformat=;;')
                            [[ "$value" =~ %[dHjklmMSuUVwWyY] ]] || warn "Directive xformat may not be in good format ($value)."
                            XFORMAT="$value"
                            verbose "XFormat set to $value."
                        fi
                    done
                    IFS="$IIFS"
                    ;;
                ...
                ;;
                YMAX )
                    [[ "$rhs" =~ ^-?[0-9]+$ || "$rhs" =~ ^-?[0-9]+\.[0-9]+$ || "$rhs" =~ ^\+?[0-9]+$ || "$rhs" =~ ^\+?[0-9]+\.[0-9]+$ || "$rhs" == "auto" || "$rhs" == "max" ]] || err "Ymax is in bad format ($rhs). Has to be 'auto'/'max'/'float'."
                    verbose "value of the YMax directive is: $rhs"
                ;;
                YMIN )
                    [[ "$rhs" =~ ^-?[0-9]+$ || "$rhs" =~ ^-?[0-9]+\.[0-9]+$ || "$rhs" =~ ^\+?[0-9]+$ || "$rhs" =~ ^\+?[0-9]+\.[0-9]+$ || "$rhs" == "auto" || "$rhs" == "min" ]] || err "Ymin is in bad format ($rhs). Has to be 'auto'/'min'/'float'."
                    verbose "value of the YMin directive is: $rhs"
                ;;
                SPEED )
                    [[ "$rhs" =~ ^\+?[1-9]([0-9])*$ || "$rhs" =~ ^\+?[1-9]([0-9])*\.[0-9]+$ || "$rhs" =~ ^\+?[0-9]+\.[1-9]([0-9])*$ || "$rhs" =~ ^\+?[0-9]+\.[0-9]*[1-9]$ ]] || err "Parameter speed is in bad format ($rhs). Has to be int/float value."
                    eval speedset=true
                ;;
                TIME )
                    [[ "$rhs" =~ ^\+?[1-9]([0-9])*$ || "$rhs" =~ ^\+?[1-9]([0-9])*\.[0-9]+$ || "$rhs" =~ ^\+?[0-9]+\.[1-9]([0-9])*$ || "$rhs" =~ ^\+?[0-9]+\.[0-9]*[1-9]$ ]] || err "Parameter time is in bad format ($rhs). Has to be int/float value."
                    eval timeset=true
                ;;
                FPS )
                    [[ "$rhs" =~ ^\+?[1-9]([0-9])*$ || "$rhs" =~ ^\+?[1-9]([0-9])*\.[0-9]+$ || "$rhs" =~ ^\+?[0-9]+\.[1-9]([0-9])*$ || "$rhs" =~ ^\+?[0-9]+\.[0-9]*[1-9]$ ]] || err "Parameter FPS is in bad format ($rhs). Has to be int/float value."
                    eval fpsset=true
                ;;
                * )
                    continue ;;
...
#    set format x "$TIMEFORMAT"
    ...
    GP=$(cat << EOF
        ...
        set title "$LEGEND"
        $GNUPLOTPARAMS
        plot '-' using 1:2:2 with $GRAPH_TYPE palette t"" 
EOF
        )   
